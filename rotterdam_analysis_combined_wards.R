...
version <- "_only_VIM_outbreak_"
folder <- "only_VIM/Outbreak"
wards <- c(1,2)
...
df_res <- NULL
for(ward in wards){
  load(file=paste0(resPath, folder, "/",ward, "/res",version,ward,".RData"))
  res <- cbind(res, ward=ward)
  df_res <- rbind(df_res, res)
} 

df_res <- df_res[, c("i","ward","alpha","beta","alphaContr","betaContr","f","meanPrev","phi","nImp","acq")]
df_res_long <- melt(df_res, id=c("i", "ward"))
...
contr_plots <- ggplot(data=df_res_long[df_res_long$variable%in%c("alphaContr","betaContr"),], 
  ...
  facet_wrap(~ward, labeller=labeller(variable=labels_routes[c(1,2)])) + 
  ...
  labs(y="Percentage (%)") + 
  scale_x_discrete(limits = rev(levels(plot_res_long$variable)[c(3,4)]),
...
contr_plots
ggsave(contr_plots, file = paste0(figPath, "contr_barplots", version, "all.pdf"), width=16, height=9)
...
imp_plots
ggsave(imp_plots, file = paste0(figPath, "imp_barplots", version, "all.pdf"), width=12, height=9)
...
sens_plots <- ggplot(df_res_long[df_res_long$variable=="phi",], aes(x=variable, y=100*value)) + 
  ...
  theme(axis.title.x = element_blank(),
        ...
        axis.ticks.x = element_blank())
sens_plots
ggsave(sens_plots, file = paste0(figPath, "sens_barplots", version, "all.pdf"), width=12, height=9)
...
prev_plots <- ggplot(df_res_long[df_res_long$variable=="meanPrev",], aes(x=variable, y=100*value)) + 
                ...
                theme(axis.title.x = element_blank(),
                      ...
                      axis.ticks.x = element_blank())
prev_plots
ggsave(sens_plots, file = paste0(figPath, "prev_barplots", version, "all.pdf"), width=12, height=9)


# Plot absolute number of importations and acquisitions for both wards
df_imp_acq <- df_res[,c("i","ward","nImp", "acq")]
df_imp_acq_long <- melt(df_imp_acq, id=c("i","ward"))
...
plot_imp_acq 
ggsave(plot_imp_acq, file = paste0(figPath, "import_acq_barplots", version, "all.pdf"), width=16, height=9)

# Plot number of positive patients divided into importations and acquisitions
df_routes <- df_res[,c("i","ward","alphaContr", "betaContr", "nImp", "acq")]
df_routes$background <- df_routes$alphaContr*df_routes$acq
df_routes$crossT <- df_routes$betaContr*df_routes$acq
df_routes <- df_routes[,c("i","ward","background", "crossT", "nImp")]
df_routes_long <- melt(df_routes, id=c("i", "ward"))
labels_routes <- c(background="Background transmission", 
                   crossT="Cross-transmission",
                   nImp = "Importations")
levels(df_routes_long$variable) <- c("background", "crossT", "nImp")
...
plot_routes <- ggplot(data=df_routes_long, aes(x=variable, y=value, fill=variable)) + 
                ...
                geom_errorbar(stat="summary", 
                              ...
                              fun.max=percentile(97.5), width=0.1) + 
                labs(y="Total number") + 
                scale_x_discrete(limits = rev(levels(df_routes_long$variable)),
                ...
                theme(axis.title.y = element_blank(),
                      ...
                      legend.position = "none")
plot_routes
ggsave(plot_routes, file = paste0(figPath, "routes_barplots", version, "all.pdf"), width=12, height=9)
